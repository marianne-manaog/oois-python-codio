        ...
        for line in self.words:
            for i in range(len(line)):
                if (i + 1) % 3 == 0:
                    ...
                    line[i] = '*' * length_of_word
        self.list_to_string()

        self.answer_file = self.words

        with open("/home/codio/workspace/code/polymorphism/answer_exercise5.txt", "w") as text_file:
            text_file.write(stars_obj.answer_file)


stars_obj = Stars(source_file=source_file, answer_file=answer_file)
stars_obj.swap_words()
print(stars_obj.answer_file)
